...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadSales() {
      if (!usingSupabase) {
        const d = await import('@/sample-data/sales.json')
        const formatted = d.default.map((s: any) => ({
  ...
  const staffMap = new Map<string, LeaderboardRow>()
  sales.forEach(s => {
    const existing = staffMap.get(s.staff_name)
    if (!existing) {
      staffMap.set(s.staff_name, { staff_name: s.staff_name, total: s.total_price, last_sale: s.created_at })
    } else {
      existing.total += s.total_price
      if (new Date(s.created_at) > new Date(existing.last_sale)) existing.last_sale = s.created_at
    }
  })
  const leaderboard = Array.from(staffMap.values()).sort((a, b) => b.total - a.total)
  ...
  const dailySales = new Map<string, number>()
  DAYS.forEach(d => dailySales.set(d, 0))
  sales.forEach(s => {
    const day = DAYS[new Date(s.created_at).getDay()]
    dailySales.set(day, (dailySales.get(day) || 0) + s.total_price)
  })
  const chartData = DAYS.map(day => ({ day, sales: dailySales.get(day) || 0 }))
  ...
  return (
    <main className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <KPICard label="Avg. Ticket" value={formatCurrency(avgTicket)} />
      </div>
